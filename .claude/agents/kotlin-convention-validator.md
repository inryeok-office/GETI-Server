...
description: "Kotlin-only. Detects and auto-fixes convention violations in changed .kt files (git diff HEAD). Exits immediately if no Kotlin files changed. Checks AGENTS.md, CLAUDE.md, .claude/rules/**, and CONTRIBUTING.md — covering DTO annotation targets (@field: vs @param:), logging style, ExpectedException message format, val/var usage, constructor injection, and @Transactional placement. Applies direct file edits for violations Spotless cannot fix, then runs spotlessApply. Outputs a list of modified files with diffs. Trigger when the user says '컨벤션 검사해줘', 'kotlin-convention-validator 실행해', or when the code-review skill is invoked. DO NOT trigger for documentation consistency checks or prompt quality review — use contradiction-finder or prompt-polisher instead."
...
**Priority when rules conflict** (AGENTS.md `## 규칙 우선순위`): `AGENTS.md` > `docs/architecture/**` > `CLAUDE.md` and `.claude/rules/**` > `docs/ai/**` and `docs/development/**` > `CONTRIBUTING.md`
...
```
to apply final formatting cleanup. Spotless runs ktlint internally; there is no `ktlintFormat` task in this repository.
